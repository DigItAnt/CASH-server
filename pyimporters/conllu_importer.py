...
def get_token(kc, username, password):
    ...
    if response.status_code != 200:
        print(Fore.RED + "Error while getting token:" + Style.RESET_ALL,
              response.text)
        sys.exit(1)
...
def create_file(filename, content, text, destination=0, token="", url=""):
    ...
    try:   
        ...
        print(Fore.RED + "Error while uploading file:", str(e) + Style.RESET_ALL)
        sys.exit(1)
    
    if response.status_code != 200:
        print(Fore.RED + "Error creating file:" + Style.RESET_ALL,
              response.text)
        sys.exit(1)
    ...
    if response.status_code != 200:
        print(Style.RED  + "Error uploading text:" + Style.RESET_ALL,
              response.text, Style.RED + "remove node" + Style.RESET_ALL, elid)
        sys.exit(1)
    print(Fore.GREEN + "File created,", len(content),
...
def create_token(docid, token_text, position, span_from, span_to, token="", url=""):
    ...
    if response.status_code != 200:
        print(Fore.RED + "Error creating token:" + Style.RESET_ALL,
              response.text)
        sys.exit(1)
    #print(response.json())
...
def create_annotation(docid, span_from, span_to, layer, value, attributes={}, token="", url=""):
    ...
    #print(response.json())
    if response.status_code != 200:
        print(Fore.RED + "Error creating annotation:" + Style.RESET_ALL,
              data,
              response.text)
        print(response)
...
def reconstruct_sentence(tokens, offset=0):
    ...
    print(Fore.GREEN + "Sentence:" + Style.RESET_ALL, ret)
    return ret
    ...
    try:
        with open(filename, "r", encoding="utf-8") as f:
            ...
            try:
                ...
                print(Fore.RED + "Error while parsing:", str(e) + Style.RESET_ALL)
                sys.exit(1)
    except IOError as e:
                print(Fore.RED + "I/O error while reading file:", str(e) + Style.RESET_ALL)
                sys.exit(1)
...
def insert(filename, destination=0, kc_token="", url="", fields=None, sentence_layer="sentences",
           verbose=False):
    print(Fore.GREEN + "Parsing file:" + Style.RESET_ALL, filename)
    ...
    print(Fore.GREEN + "Inserting doc into " + url + ":" + Style.RESET_ALL +
          "\n", text[:50] + "...")
    docid, srcid = create_file(filename, raw, text, destination, kc_token, url)
        ...
        for token in sentence:
            ...
            for coli, col in enumerate(['form', 'lemma', 'upos', 'xpos',
                ...
                if col == 'form': # insert the token
                    print(Fore.GREEN + "Inserting token:" + Style.RESET_ALL,
                          token[col] + Fore.GREEN + "@" +
                          str(span_from) + ":" + str(span_to), end = '')
                    if '__skip__' in token: tokencount -= 1
                    tokenid = create_token(docid, token[col], tokencount, span_from, span_to, kc_token, url)
                    tokencount += 1
                elif col == 'feats' or col == 'misc': # insert the features
                    layer = fields[coli-1] if fields else col
                    value = feat2str(token[col])
                    if value != '' and value is not None:
                        if verbose: print("\nannotation:", layer, value, end='')
                        create_annotation(docid, span_from, span_to, layer, value, token[col], kc_token, url)
                        ann_cnt += 1
                    ...
                    if value != '' and value is not None:
                        if verbose: print("\nannotation:", layer, value, end='')
                        create_annotation(docid, span_from, span_to, layer, value, {}, kc_token, url)
                        ann_cnt += 1
                    if value != None:
                        create_annotation(docid, span_from, span_to, layer, value, {}, kc_token, url)
                        ann_cnt += 1
            if verbose: print(Style.RESET_ALL)
            else: print(" with", ann_cnt, "annotations" + Style.RESET_ALL)

        # insert the sentence annotation
        layer = sentence_layer
        value = "sentence"
        create_annotation(docid, sentence_start, sentence_end, layer, value, sentence.metadata, kc_token, url)
        print(Fore.BLUE + Style.BRIGHT + "Inserting sentence annotation:" + Style.RESET_ALL,
...
if __name__ == "__main__":
    ...
    argparser.add_argument("-s", "--sentence-layer", help="name of the sentence layer (default: sentences)",
                           default="sentences")
    argparser.add_argument("-d", "--destination", default=0, help="folder destination ID")
    ...
    if ( args.fields ):
        ...
        if len(fields) != 8:
            print(Fore.RED + "Fields must be 8 comma-separated values for " +
    ...
    try:
        ...
        print(Fore.RED + "Error while getting OAuth token:", str(e) + Style.RESET_ALL)
        sys.exit(1)
            
    insert(args.filename, destination=args.destination, kc_token=token,
           url=args.cash_url, fields=args.fields, sentence_layer=args.sentence_layer,
